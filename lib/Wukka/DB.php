...
* use a component from wukka.  We're not gonna try to force you to rewrite your entire application
* just to be able to use something in wukka that communicates with a database. Instead, we'd like 
...
* adapter. We can do this easily since the Wukka\DB object really only needs to provide just a handful 
...
class DB implements ExtendedIface {
    ...
    * If you need another adapter, email John Loehrer <jloehrer@wukkaonline.com> or submit your own.
    * Thinking about writing an adapter for DBAL -- doctrine's data access layer.
    */
    function __construct( $core ){
    
        while( $core instanceof \Wukka\DB && $core->core() instanceof Iface ) $core = $core->core();
    ...
    *
    * $db = new Wukka\DB( $mysqli );
    * $db->isa('mysql'); // returns true.
    * 
    * $db = new Wukka\DB( $pdo_mysql );
    * $db->isa('mysql'); // returns true.
    */
